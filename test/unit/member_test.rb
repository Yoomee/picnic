...
class MemberTest < ActiveSupport::TestCase
  ...
  context "on destruction" do
    ...
    setup do
      @member = Factory.create(:member)
      @photo_album = Factory.create(:photo_album, :attachable => @member, :system_album => false, :id => 986)
      @system_album = Factory.create(:photo_album, :attachable => @member, :system_album => true, :id => 987)
      @member.destroy
    end
    
    should "delete photo albums" do
      assert !PhotoAlbum.exists?(986)
    end
    
    should "not delete system photo albums" do
      assert PhotoAlbum.exists?(987)
    end
