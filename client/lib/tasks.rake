...
namespace :picnic do
  task :sqlite => :environment do
    ...
    db.execute2("CREATE TABLE ZMEMBER ( Z_PK INTEGER PRIMARY KEY, Z_ENT INTEGER, Z_OPT INTEGER, ZFORENAME STRING, ZSURNAME STRING, ZBIO STRING, ZIS_SPEAKER BOOLEAN)")
    ...
    Venue.all.each do |venue|
      db.execute2("INSERT INTO ZVENUE VALUES (#{venue.id},2,1,'#{quote_string(venue.name)}',#{venue.weight})")
    end
      ...
      if !session.tags.blank?
        ...
        session.speakers.each do |speaker|
          if !inserted_speakers.include?(speaker)
            db.execute2("INSERT INTO ZMEMBER VALUES (#{speaker.id},2,1,'#{quote_string(speaker.forename)}', '#{quote_string(speaker.surname)}', '#{quote_string(speaker.bio)}', 1)")
