...
namespace :picnic do
  task :sqlite => :environment do
    ...
    db.execute2("CREATE TABLE ZCONFERENCESESSION ( Z_PK INTEGER PRIMARY KEY, Z_ENT INTEGER, Z_OPT INTEGER, ZVENUE INTEGER, ZSTARTSAT TIMESTAMP, ZTIMESTAMP TIMESTAMP, ZENDSAT TIMESTAMP, ZNAME VARCHAR, ZTEXT VARCHAR, ZCOLORR FLOAT, ZCOLORG FLOAT, ZCOLORB FLOAT)")
    ...
    ConferenceSession.all.each_with_index do |session, idx|
      db.execute2("INSERT INTO ZCONFERENCESESSION VALUES (#{session.id},2,1,#{session.venue_id}, '#{timestamp(session.starts_at)}', '#{timestamp(session.created_at)}', '#{timestamp(session.ends_at)}', '#{quote_string(session.name)}', '#{quote_string(session.description)}', #{hex_to_rgb(session.color).join(',')})")
    end
  end  
end
