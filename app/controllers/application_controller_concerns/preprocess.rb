module ApplicationControllerConcerns::Preprocess
  ...
  def find_logged_in_member
    ...
    elsif cookies[:remember_me_id]
      member_id = cookies[:remember_me_id].to_i - ApplicationController::REMEMBER_ME_SALT
      remembered_member = Member.find(member_id)
      if (remembered_member && Digest::SHA1.hexdigest(remembered_member.email)[4,18] == cookies[:remember_me_code])
        session[:logged_in_member_id] = remembered_member.id
        remembered_member
      end
    end
  end
  ...
  private
  def store_remember_me_cookies(member)
    cookies[:remember_me_id] = {:value => member.id + ApplicationController::REMEMBER_ME_SALT, :expires => 1.minute.from_now}
    member_code = Digest::SHA1.hexdigest(member.email)[4,18]
    cookies[:remember_me_code] = {:value => member_code, :expires => 1.minute.from_now}
  end
  
  def delete_remember_me_cookies!
    if cookies[:remember_me_id]
      cookies.delete :remember_me_id 
    end  
    if cookies[:remember_me_code]
      cookies.delete :remember_me_code
    end
  end
  
end
ApplicationController::REMEMBER_ME_SALT = 12874