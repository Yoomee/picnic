...
module ApplicationControllerConcerns::Twitter
  ...
  def fetch_latest_tweets_for(screen_name, options = {})
    ...
    begin
      ...
      if RAILS_ENV=='development'
        ...
        if options[:list_id]
          url = "https://api.twitter.com/1.1/#{screen_name}/lists/#{options[:list_id]}/statuses.json?per_page=#{options[:limit].to_i+1}&include_entities=false"
        else
          url = "https://api.twitter.com/1.1/statuses/user_timeline.json?count=#{options[:limit].to_i*15}&include_entities=false&screen_name=#{screen_name}&include_rts=true&exclude_replies=true"
        end
        tweets_json = open(url, "Authorization" => "Bearer #{APP_CONFIG[:twitter_bearer_token]}").read
  ...
  def fetch_latest_tweets_from(user, limit, replies, with_dates)
    fetch_latest_tweets_for(user, :limit => limit).collect{|t| with_dates ? t : t.text}
  end
