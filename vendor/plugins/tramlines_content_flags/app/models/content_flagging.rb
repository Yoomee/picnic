class ContentFlagging < ActiveRecord::Base
  ...
  belongs_to :flagger, :class_name => "Member", :foreign_key => "member_id"
  ...
  named_scope :from_today, lambda {{:conditions => ["content_flaggings.created_at > ?", Date.today]}}
  named_scope :from_different_people, :group => "content_flaggings.ip_address, content_flaggings.member_id"
  ...
  def create_content_flag_fields_and_unresolve
    if flagged_by_human? && has_attachable?
      if attachable.class.respond_to?(:filtered_attributes)
        ...
        attachable.class.columns.collect{|col|col.name if (col.type.in?([:text, :string]) && !col.name.match(/type$/))}.compact.each do |attribute|
          content_flag.create_content_flag_field_if_changed(attribute, attachable.send(attribute))
        end
      end
      if attachable.respond_to?(:member_id) && attachable.member_id
        content_flag.create_content_flag_field_if_changed("member_id", attachable.member_id)
      end
    end
