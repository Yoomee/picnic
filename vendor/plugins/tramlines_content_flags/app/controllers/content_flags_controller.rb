class ContentFlagsController < ContentFlagsBaseController
  ...
  def create
    @content_flag = ContentFlag.new(params[:content_flag].merge(:member => @logged_in_member))
  ...
  def remove
    ...
    if @content_flag.removable?
      @content_flag.attachable.update_attributes(:removed_by => @logged_in_member, :removed_at => Time.now)
    end
  ...
  def resolve
    @content_flag = ContentFlag.find(params[:id])
    @content_flag.resolve!(@logged_in_member)
